...
- **zhipu CogView 注记**：中英皆可、中文理解好；一句话一个要素，50-100 字，过长画面发糊；
  免费档带水印（2026-09 实测，以 `watermark_profiles.json` 为准）→ 产物只作风格参考/概念验证，不入正片
...
- **定位升级（2026-09-06 cfg 修复后，摘"垃圾"帽）**：从"仅 kenburns 伪运动底片"升级为**本地免费真运动视频档**——运镜连贯已实测（前推/场景保持），可当补镜头、氛围镜、i2v 兜底；画质仍逊顶级商模级，主力片依旧走高规格视频模型
...
- **i2v 直产 mp4**：5.04s / 1088×832 (4:3) / 24fps / H.264 + AAC 立体声音轨（自带音频，无需 webp2mp4 兜底）
- **保留首帧构图**（不像 ltx 重绘），prompt 主要引导运动方向与变化 → 首帧投入回报高
- 1 RPM/把 key（MEDIA_AGNES_1/2/3），3 把 key 并行 = 3 RPM 满载（自带节流：per-key 间隔 ≥ 62s 安全）
- 轮询有 read timeout 风险，但脚本兜底 OK 不丢任务
- 适合：广告片 AIGC 赛道、产品宣传、商业短片；速度比 ltx 快、规格比 ltx 长、音轨免后期
- prompt 喂法：照卡一·活C（短句、慢速控制词、locked camera 稳画面、单事件）
- 实测输出无水印（2026-09-05，"自定义 Agnes key"档位；复核看 `watermark_profiles.json`）

### 实测卡 · Qwen/Qwen-Image（custom_1，2026-09-05 沉淀）

- 异步任务式，魔搭 API（MEDIA_CUSTOM_1），500次/天，单张 20-40s
- **中文文字渲染招牌级**：招牌/标签/包装上的中文 4-6 字无错字、笔画自然（如「桂花糕」「江南古镇」）
- prompt 必加引号 + 明确指出"木质招牌/标签/包装盒上写着汉字 XX" + 字号位置提示
- **特写型 prompt 会挤压"小卡片"类元素**（产品特写里"附带小标签"容易丢失）→ 卡片类文字用中景/全景构图保证空间
- 写实摄影风强，写中文文字时要强调"with detailed background"避免背景被虚化压扁
- 适合：地理标志产品、商业广告文案、产品包装、品牌物料
